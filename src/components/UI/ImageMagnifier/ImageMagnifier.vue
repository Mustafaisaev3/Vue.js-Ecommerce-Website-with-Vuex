<template>
    <div class="image-magnifier relative" :style="style">
        <img 
             :width="width"
             :height="heigth"
             :src="src"
             ...
             @mousewheel="handleWhellScroll"
             ref="img"
        />
        <div class="image-magnifier__mask" :class="maskClass" :style="maskStyle" ref="mask">
        </div>
        <div class="image-magnifier__zoom border-[#4f4f4f] border-[1px]" :class="zoomClass" :style="zoomStyle" v-show="zoomShow">
            <img class="max max-w-none" :src="zoomSrc" :style="zoomImgStyle"/>
        </div>
    </div>
</template>
  ...
  export default {
    ...
    computed: {
      ...
      maskStyle() {
        return {
          position: 'absolute',
          width: `${this.maskElemWidth}px`,
          height: `${this.maskElemHeight}px`,
    ...
    methods: {
      ...
      handleWhellScroll(e) {
        e.stopPropagation()
        if (e.wheelDelta > 0) {
          this.maskElemWidth += 100
          this.maskElemHeight += 100
